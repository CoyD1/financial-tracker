...
const COLORS = ['#3B82F6', '#10B981', '#F59E0B', '#6366F1', '#8B5CF6', '#EC4899', '#EF4444'];
...
const Dashboard = () => {
  ...
  const fetchDashboardData = useCallback(async () => {
    try {
      const [statsResponse, transactionsResponse, goalsResponse] = await Promise.all([
        api.get("stats/"),
      ...
      const summary = stats.summary || { total_income: 0, total_expense: 0, balance: 0 };
  ...
  const handleAddContribution = async (goalId) => {
    ...
    try {
      ...
      setUserData(prev => ({ ...prev, contributionAmount: '', showGoalModal: false }));
      fetchDashboardData();
    } catch (error) {
  ...
  const activePieData = userData.pieType === 'EX' ? userData.expensePieData : userData.incomePieData;
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="bg-zinc-900/50 p-6 rounded-2xl border border-zinc-800 relative overflow-hidden group hover:border-blue-500/50 transition-all hover:shadow-[0_0_20px_rgba(59,130,246,0.1)]">
            <div className="flex justify-between items-start mb-4 relative z-10">
              <p className="text-zinc-500 text-xs font-semibold uppercase tracking-widest">Баланс</p>
              <div className="p-2 bg-blue-500/10 rounded-lg text-blue-500 border border-blue-500/20">
                <Wallet size={18} />
              </div>
            </div>
            <div className="relative z-10 mb-6 h-8 flex items-end">
              <h3 className={`font-bold text-white tracking-tight transition-all duration-300 ${getFontSizeClass(userData.balance)}`}>
          ...
          <div className="bg-zinc-900/50 p-6 rounded-2xl border border-zinc-800 group hover:border-emerald-500/50 transition-all hover:shadow-[0_0_20px_rgba(16,185,129,0.1)] relative overflow-hidden">
            <div className="flex justify-between items-start mb-4 relative z-10">
              <p className="text-zinc-500 text-xs font-semibold uppercase tracking-widest">Доходы</p>
              <div className="p-2 bg-emerald-500/10 rounded-lg text-emerald-500 border border-emerald-500/20">
                <TrendingUp size={18} />
              </div>
            </div>
            <div className="relative z-10 h-8 flex items-end">
              <h3 className={`font-bold text-white tracking-tight transition-all duration-300 ${getFontSizeClass(userData.income)}`}>
          ...
          <div className="bg-zinc-900/50 p-6 rounded-2xl border border-zinc-800 group hover:border-red-500/50 transition-all hover:shadow-[0_0_20px_rgba(239,68,68,0.1)] relative overflow-hidden">
            <div className="flex justify-between items-start mb-4 relative z-10">
              <p className="text-zinc-500 text-xs font-semibold uppercase tracking-widest">Расходы</p>
              <div className="p-2 bg-red-500/10 rounded-lg text-red-500 border border-red-500/20">
                <TrendingDown size={18} />
              </div>
            </div>
            <div className="relative z-10 h-8 flex items-end">
              <h3 className={`font-bold text-white tracking-tight transition-all duration-300 ${getFontSizeClass(userData.expense)}`}>
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex gap-4">
              <span className="flex items-center gap-2 text-xs text-zinc-400">
                <span className="w-2 h-2 rounded-full bg-blue-600 shadow-[0_0_10px_rgba(37,99,235,0.5)]"></span> Доход
              </span>
              <span className="flex items-center gap-2 text-xs text-zinc-400">
                <span className="w-2 h-2 rounded-full bg-purple-500 shadow-[0_0_10px_rgba(147,51,234,0.5)]"></span> Расход
              </span>
            </div>
          </div>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={userData.chartData}>
                ...
                <YAxis stroke="#52525b" tick={{ fill: '#71717a', fontSize: 12 }} axisLine={false} tickLine={false} dx={-10} />
                <Tooltip
                  contentStyle={{ backgroundColor: '#09090b', borderColor: '#27272a', borderRadius: '0.75rem', color: '#f4f4f5' }}
                  itemStyle={{ color: '#f4f4f5' }}
                />
                <Area name="Доход" type="monotone" dataKey="income" stroke="#2563EB" strokeWidth={3} fillOpacity={1} fill="url(#colorIncome)" />
                <Area name="Расход" type="monotone" dataKey="expense" stroke="#9333EA" strokeWidth={3} fillOpacity={1} fill="url(#colorExpense)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-zinc-900 p-6 rounded-2xl border border-zinc-800">
          ...
          <div className="overflow-x-auto">
                  ...
                  <tr key={t.id} className="group hover:bg-zinc-800/50 transition-colors">
                    <td className="py-4">
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center border ${t.type === 'IN' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : 'bg-red-500/10 border-red-500/20 text-red-500'
                          }`}>
                          {t.type === 'IN' ? <TrendingUp size={18} /> : <TrendingDown size={18} />}
                        </div>
                        <div>
                          <p className="font-medium text-zinc-200 group-hover:text-white transition-colors">{t.description || 'Без описания'}</p>
                          <p className="text-xs text-zinc-500">Завершено</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-4 text-zinc-400 text-sm">{t.category_name}</td>
                    <td className="py-4 text-zinc-400 text-sm">{new Date(t.date).toLocaleDateString()}</td>
                    <td className={`py-4 text-right font-medium ${t.type === 'IN' ? 'text-emerald-400' : 'text-zinc-100'}`}>
            ...
            {userData.recentTransactions.length === 0 && (
              <div className="text-center py-8 text-zinc-500">Операций не найдено</div>
            )}
          </div>
        </div>
        ...
        <div className="bg-zinc-900 p-6 rounded-2xl border border-zinc-800">
          <div className="flex flex-col gap-6 mb-6">
            ...
            <div className="flex p-1 bg-zinc-950 border border-zinc-800 rounded-xl">
              <button
                onClick={() => setUserData(prev => ({ ...prev, pieType: 'EX' }))}
                className={`flex-1 py-1.5 text-xs font-bold uppercase tracking-wider rounded-lg transition-all ${userData.pieType === 'EX'
              ...
              <button
                onClick={() => setUserData(prev => ({ ...prev, pieType: 'IN' }))}
                className={`flex-1 py-1.5 text-xs font-bold uppercase tracking-wider rounded-lg transition-all ${userData.pieType === 'IN'
                  ? 'bg-zinc-800 text-white shadow-lg'
                  : 'text-zinc-500 hover:text-zinc-300'
                  }`}
              >
          ...
          <div className="h-[250px] w-full relative">
            {activePieData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart data={activePieData} key={`pie-${userData.pieType}-${activePieData.length}`}>
                  <Pie
                    data={activePieData}
                    cx="50%"
                    cy="50%"
              ...
              <div className="flex flex-col items-center justify-center h-full text-zinc-500 space-y-2 border-2 border-dashed border-zinc-800 rounded-3xl">
                ...
                <span className="text-[10px] uppercase tracking-widest font-bold">Данных пока нет</span>
              </div>
            )}
            ...
            {activePieData.length > 0 && (
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                ...
                <span className="text-white font-bold text-xl">
                  {formatCurrency(userData.pieType === 'EX' ? userData.expense : userData.income)}
                </span>
              </div>
            )}
          </div>
            ...
            {activePieData.slice(0, 6).map((entry, index) => (
              <div key={index} className="flex justify-between items-center text-sm group">
                <div className="flex items-center gap-3">
                  <div className="w-2.5 h-2.5 rounded-full ring-4 ring-zinc-900" style={{ backgroundColor: COLORS[index % COLORS.length] }}></div>
                  <span className="text-zinc-400 group-hover:text-zinc-200 transition-colors font-medium">{entry.name}</span>
                </div>
                <span className="text-zinc-100 font-bold">{formatCurrency(entry.value)}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-zinc-900 p-6 rounded-2xl border border-zinc-800 relative overflow-hidden">
          ...
          <div className="flex justify-between items-center mb-6 relative z-10">
            ...
            {userData.goals.length > 0 && (
              <span className={`px-2.5 py-1 border text-xs font-medium rounded-lg transition-colors ${parseNum(userData.goals[0].current_saved) >= parseNum(userData.goals[0].target_amount)
                  ? 'bg-blue-500/10 text-blue-400 border-blue-500/20'
                  : 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20'
                }`}>
                {parseNum(userData.goals[0].current_saved) >= parseNum(userData.goals[0].target_amount) ? 'Выполнено' : 'В процессе'}
              </span>
            )}
          </div>
          ...
          {userData.goals.length > 0 ? (
            <>
              {userData.goals.filter(g => g.is_active).slice(0, 1).map(goal => {
                const progress = Math.min(Math.round((parseNum(goal.current_saved) / parseNum(goal.target_amount)) * 100), 100);
                return (
                  <div key={goal.id}>
                    <div className="mb-2 flex justify-between text-sm relative z-10">
                      <span className="text-zinc-400">Цель: {goal.title}</span>
                    ...
                    <div className="flex justify-between text-xs text-zinc-500 relative z-10">
                      <span>{formatCurrency(goal.current_saved)} накоплено</span>
                    ...
                    {userData.showGoalModal ? (
                      <div className="mt-6 p-4 bg-zinc-950 rounded-xl border border-zinc-800 animate-in fade-in slide-in-from-top-2">
                        <input
                          ...
                          onChange={(e) => setUserData(prev => ({ ...prev, contributionAmount: e.target.value }))}
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            onClick={() => setUserData(prev => ({ ...prev, showGoalModal: false }))}
                      ...
                      <button
                        onClick={() => setUserData(prev => ({ ...prev, showGoalModal: true }))}
            ...
            <div className="text-center py-8 space-y-4">
              ...
              <p className="text-zinc-500 text-sm">У вас пока нет активных целей</p>
              ...
              {userData.showCreateGoalModal ? (
                <div className="mt-4 p-4 bg-zinc-950 rounded-xl border border-zinc-800 animate-in fade-in slide-in-from-top-2 text-left">
                  <input
                    ...
                    onChange={(e) => setUserData(prev => ({ ...prev, newGoalTitle: e.target.value }))}
                  ...
                  <input
                    ...
                    onChange={(e) => setUserData(prev => ({ ...prev, newGoalTarget: e.target.value }))}
                    className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-2 text-white mb-3 focus:outline-none focus:border-blue-500"
                  />
                  <div className="flex gap-2">
                    <button
                    ...
                    <button
                      onClick={() => setUserData(prev => ({ ...prev, showCreateGoalModal: false }))}
                      className="bg-zinc-800 hover:bg-zinc-700 text-zinc-400 px-4 py-2 rounded-lg text-sm transition-all"
                    >
                      Отмена
                    </button>
                ...
                <button
                  onClick={() => setUserData(prev => ({ ...prev, showCreateGoalModal: true }))}
